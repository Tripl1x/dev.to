...
class Downloader:
    ...
    def save_images(cls, links) -> None:
        ...
        for link in links:
            ...
            with open(f'{cls.image_path}/{filename}', 'wb') as file:
                file.write(r.content)

    @classmethod
    def save_user(cls, user):
        """"Функция для сохранения пользователя в json файл"""
        with open(f'{cls.path}/user.json', 'w') as file:
    ...
    def save_comments(cls, comments):
        ...
        with open(f'{cls.path}/comments.json', 'w') as file:
